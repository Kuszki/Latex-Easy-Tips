...
Wykresy generować można również bezpośrednio w dokumencie, stosując w tym celu bibliotekę \texttt{pgfplots}. Rozwiązanie to najlepiej integruje się z dokumentem, a dodatkowo wykresy są automatycznie generowane podczas jego kompilacji, co wyklucza konieczność ręcznego uruchamiania generujących ich skryptów. Przykład wykresu, który generowany jest na podstawie listingu~\ref{lst:pgfplots_demo} przedstawia rysunek~\ref{fig:pgfplots_demo}, przy czym osiągnięty efekt jest podobny do rysunku~\ref{fig:gnuplot_demo}. Wartości osi są formatowane zgodnie z ustawieniami przedstawionymi wcześniej w listingu~\ref{lst:pgfplots_config}. Tworząc wykres stosować można wszystkie funkcje dostępne w dokumencie. Wykresy sporządzone w ten sposób cechują się najwyższą jakością.

\begin{listing}[hbt]
\inputminted{latex}{obrazki/pgfplots_demo.tex}
\makecaption{lst:pgfplots_demo}{Skrypt programu \texttt{gnuplot} generujący rysunek~\ref{fig:gnuplot_demo}}
\end{listing}
